  ...
  describe '#perform' do
    context 'without scope' do
      it 'runs the :reload action' do
        expect(Guard).to receive(:async_queue_add).with([:guard_reload, { groups: [], plugins: [] }])
    ...
    context 'with a valid Guard group scope' do
      it 'runs the :reload action with the given scope' do
        expect(Guard).to receive(:async_queue_add).with([:guard_reload, { groups: [foo_group], plugins: [] }])
    ...
    context 'with a valid Guard plugin scope' do
      it 'runs the :reload action with the given scope' do
        expect(Guard).to receive(:async_queue_add).with([:guard_reload, { plugins: [bar_guard], groups: [] }])
